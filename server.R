...
primatrixnames = Data_Primatrix$Name_Matrix #still necessary?
additivenames = Data_Additive$Name_Matrix #still necessary?
fillernames = Data_Filler$Name_Matrix #still necessary?
Data_othermatrix <- subset(Data_MatrixM, Type_Matrix != "Insert") #still necessary?
othermatrixnames = Data_othermatrix$Name_Matrix # still necessary?

#Name Data_Inserts columns
Data_Insert =  read.csv("data/Data_Inserts.csv")
insertsnames = Data_Insert$Name_Insert
insertsenergy = Data_Insert$Energy_Insert
...
finishenergy = Data_Finish$Energy_Finishing
...
Data_Primatrix_new  <- reactiveValues()
  ...
  # OtherMat ----
  # Build temp and new data tables for if adding custom data
 ...
 output$testtable <- renderTable(Data_MatrixM_new())
 ...
 observeEvent(input$types1a, {
  ...
  # Associate Name/Energy with Matrix type name
  ...
  updateSelectizeInput(session, 'InsertsBInput2',
                       ...
                       selected = "Not Used",
                       server = TRUE)
  # Make Inserts lists if custom data
  ...
  Data_Cure_new  <- eventReactive(input$gocure, {
    ...
    cure.Ener <- whichcureenergy(input$cure_add_EYN, input$cure_add_E_P, input$cure_add_E_per, input$cure_add_E_t, input$cure_add_E_m, input$cure_add_E_Y)
  ...
  curenamefetch1 <- eventReactive(input$cureInput1,{
    fn_name <- whichone(input$gocure, Data_Cure_new()$Name_Cure, Data_Cure$Name_Cure)
    fn_name[fn_name %in% input$cureInput1]
  })
  output$curename1d  <- output$curename1 <- renderText(curenamefetch1())
  
  curenamefetch2 <- eventReactive(input$cureInput2,{
    fn_name <- whichone(input$gocure, Data_Cure_new()$Name_Cure, Data_Cure$Name_Cure)
    fn_name[fn_name %in% input$cureInput2]
  })
  ...
  cureenergyfetch1 <- eventReactive(input$cureInput1,{
    fn_name <- whichone(input$gocure, Data_Cure_new()$Name_Cure, Data_Cure$Name_Cure)
    fn_energy <- whichone(input$gocure, Data_Cure_new()$Energy_Cure, Data_Cure$Energy_Cure)
    fn_energy[fn_name %in% input$cureInput1]
  })
  ...
  cureenergyfetch2 <- eventReactive(input$cureInput2,{
    fn_name <- whichone(input$gocure, Data_Cure_new()$Name_Cure, Data_Cure$Name_Cure)
    fn_energy <- whichone(input$gocure, Data_Cure_new()$Energy_Cure, Data_Cure$Energy_Cure)
    fn_energy[fn_name %in% input$cureInput2]
  })
 ...
 observeEvent(input$othermatrixCUSERYN2, {
   ...
   energy_plot.df.p <- reactive({
     energy_plot.df() %>%
       mutate(techset = as.factor(techset),
              process_segment = reorder(process_segment, order))
   })

   output$plot1 <- renderPlotly({
    ggplot(energy_plot.df.p(), aes(x = techset, y = process_energy, fill = process_segment )) +
